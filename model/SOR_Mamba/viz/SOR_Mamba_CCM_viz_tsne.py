...
class Model(nn.Module):
    ...
    def random_mask(self,x_,p=0.2):
        ...
        N = int(x_.shape[-1]*p)
        idx_mask = torch.randperm(x_.shape[-1])[:N]
    ...
    def pretrain2(self, x_enc, x_mark_enc,train,mi=False):
        if self.use_norm:
            means = x_enc.mean(1, keepdim=True).detach()
            x_enc = x_enc - means
            stdev = torch.sqrt(torch.var(x_enc, dim=1, keepdim=True, unbiased=False) + 1e-5)
        ...
        _, _, N = x_enc.shape # B L N
        #x_enc_masked, mask = self.random_mask(x_enc,p=self.p)
        corr_gt = self.compute_correlation_matrix(x_enc)
        
        #enc_out = self.enc_embedding(x_enc_masked, x_mark_enc) # covariates (e.g timestamp) can be also embedded as tokens
        enc_out = self.enc_embedding(x_enc, x_mark_enc) # covariates (e.g timestamp) can be also embedded as tokens
        ...
        if self.use_norm:
            enc_out = enc_out * (stdev[:, 0, :].unsqueeze(1).repeat(1, self.seq_len, 1))
            enc_out = enc_out + (means[:, 0, :].unsqueeze(1).repeat(1, self.seq_len, 1))
        
        return enc_out,loss_reg,mask
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec,train,mi=False):
        if self.use_norm:
            ...
            x_enc /= stdev
        _, _, N = x_enc.shape # B L N
        enc_out1 = self.enc_embedding(x_enc, x_mark_enc) # covariates (e.g timestamp) can be also embedded as tokens
        enc_out2 = self.enc_embedding(torch.flip(x_enc,dims=[2]), x_mark_enc)
        enc_out1, attns,loss_reg = self.encoder(enc_out1, attn_mask=None,train=train,mi=mi)
        enc_out2, attns,loss_reg = self.encoder(enc_out2, attn_mask=None,train=train,mi=mi)
        enc_out1 = enc_out1[:,:N,:]
        enc_out2 = enc_out2[:,:N,:]
        return enc_out1, torch.flip(enc_out2,dims=[1])
    ...
    def forward(self, x_enc, x_mark_enc, x_dec=0, x_mark_dec=0, mask=None,train=True,mi=False):
        enc1, enc2 = self.forecast(x_enc, x_mark_enc, x_dec, x_mark_dec,train,mi)
        return enc1, enc2